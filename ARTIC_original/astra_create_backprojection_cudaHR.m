function backProj = astra_create_backprojection_cudaHR(sinogramData, proj_geom, vol_geom)
%--------------------------------------------------------------------------
...
backProj = zeros(vol_geom.GridRowCount, vol_geom.GridColCount);

for b = 1:length(proj_geoms)    
    sinogram_id = astra_mex_data2d('create', '-sino', proj_geoms{b}, sinogramData(indices(b*2-1):indices(b*2),:));
    astra_mex_data2d('store', recon_id, 0);
