function [sino] = astra_create_sino_cudaHR(data, proj_geom, vol_geom, gpu_index)
...
sino = zeros(nbProjections, proj_geom.DetectorCount);
for b = 1:length(proj_geoms)    
    % store sino
    sino_id = astra_mex_data2d('create','-sino', proj_geoms{b}, 0);
    ...
    sino(indices(b*2-1):indices(b*2),:) = astra_mex_data2d('get',sino_id);
